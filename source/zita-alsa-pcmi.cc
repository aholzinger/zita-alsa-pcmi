...
#error "System byte order is undefined or not supported"
    ...
    if (_capt_handle) {
        ...
        if (snd_pcm_hw_params_get_periods(_capt_hwpar, &nfrag, &dir) || (nfrag != _capt_nfrag) || dir) {
            if (_debug & DEBUG_INIT)
                fprintf(stderr, "Alsa_pcmi warning: requested %u periods for capture, using %u.\n", _capt_nfrag, nfrag);
        }
...
int Alsa_pcmi::set_hwpar(snd_pcm_t *handle, snd_pcm_hw_params_t *hwpar, const char *sname, unsigned int nfrag, unsigned int *nchan) {
    ...
    if (snd_pcm_hw_params_set_periods_integer(handle, hwpar) < 0) {
        if (_debug & DEBUG_INIT)
            fprintf(stderr, "Alsa_pcmi: can't set %s periods to integral value.\n",
                    sname);
        return -1;
    }
    if ((snd_pcm_hw_params_set_access(handle, hwpar, SND_PCM_ACCESS_MMAP_NONINTERLEAVED) < 0) && (snd_pcm_hw_params_set_access(handle, hwpar, SND_PCM_ACCESS_MMAP_INTERLEAVED) < 0) && (snd_pcm_hw_params_set_access(handle, hwpar, SND_PCM_ACCESS_MMAP_COMPLEX) < 0)) {
        if (_debug & DEBUG_INIT)
    ...
    if (_debug & FORCE_16B) {
        err = (snd_pcm_hw_params_set_format(handle, hwpar, SND_PCM_FORMAT_S16_LE) < 0) && (snd_pcm_hw_params_set_format(handle, hwpar, SND_PCM_FORMAT_S16_BE) < 0);
    } else {
        err = (snd_pcm_hw_params_set_format(handle, hwpar, SND_PCM_FORMAT_FLOAT_LE) < 0) && (snd_pcm_hw_params_set_format(handle, hwpar, SND_PCM_FORMAT_FLOAT_BE) < 0) && (snd_pcm_hw_params_set_format(handle, hwpar, SND_PCM_FORMAT_S32_LE) < 0) && (snd_pcm_hw_params_set_format(handle, hwpar, SND_PCM_FORMAT_S32_BE) < 0) && (snd_pcm_hw_params_set_format(handle, hwpar, SND_PCM_FORMAT_S24_3LE) < 0) && (snd_pcm_hw_params_set_format(handle, hwpar, SND_PCM_FORMAT_S24_3BE) < 0) && (snd_pcm_hw_params_set_format(handle, hwpar, SND_PCM_FORMAT_S16_LE) < 0) && (snd_pcm_hw_params_set_format(handle, hwpar, SND_PCM_FORMAT_S16_BE) < 0);
    }
    if (err) {
        if (_debug & DEBUG_INIT)
