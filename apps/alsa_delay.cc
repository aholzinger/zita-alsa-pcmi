...
public:
    Audiothr(const char *playdev, const char *captdev,
             int fsamp, int frsize, int playnfrags, int captnfrags,
             int inp, int out, int debug = 0);
...
Audiothr::Audiothr(const char *playdev, const char *captdev,
    ...
    _pcmi = new Alsa_pcmi(playdev, captdev, nullptr, fsamp, frsize, playnfrags, captnfrags, debug);
    if (_pcmi->state() != Alsa_pcmi::STATE_OPEN) {
...
int main(int ac, char *av[]) {
    int fsamp, frsize, playnfrags, captnfrags, inp, out, debug;
    MTDM *mtdm;
    float t;

    if (ac < 8) {
        fprintf(stderr, "alsa-delay <playdev> <captdev> <fsamp> <frsize> <playnfrags> <captnfrags> <input> <output>\n");
    ...
    audio = new Audiothr(av[1], av[2], fsamp, frsize, playnfrags, captnfrags, inp, out, debug);
    mtdm  = audio->mtdm();
    t     = 1000.0f / fsamp;
